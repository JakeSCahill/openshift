...
Then, you can access Management Center by opening `management-center-service-hazelcast.192.168.99.100.nip.io/hazelcast-mancenter` (`/mancenter` for versions prior to `3.10`) in your browser.
...
In the main project directory run the following command to install the snapshot JAR files from the root directory:
...
[source, shell]
----
mkdir hazelcast-configuration/ && cp ../../client-apps/ocp-entry-processor/target/ocp-entry-processor-0.1-SNAPSHOT.jar
...
image::../assets/images/entry_processor.png[Entry Processor]

The results should be visible in Management Center.

image::../assets/images/entry_processor_mancenter.png[Management Center]
